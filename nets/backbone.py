...
from .dcn import DCNv3_pytorch,DCV2_x
...
class OverlapPatchEmbed(nn.Module):
    def __init__(self, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        ...
        # self.dcn = DCNv3_pytorch(embed_dim,kernel_size=patch_size[0],pad=patch_size[0]//2)
        self.dcn = DCV2_x(embed_dim,group=4)
        # self.globa_trans = DAttentionBaseline((32,32), (32,32), n_heads=4, n_head_channels=embed_dim//4, n_groups=4,
        ...
        #     DCNv3_pytorch(embed_dim,kernel_size=patch_size[0],pad=patch_size[0]//2)
        # ])
    ...
    def forward(self, x):
        x = self.proj(x)
        x = self.dcn(x)# + self.globa_trans(x)
        _, _, H, W = x.shape
...
class DAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,offset_range_factor=1):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
        ...
        self.q_offset = nn.Linear(dim, 2, bias=qkv_bias)
    ...
    def forward(self, x, H, W):
        ...
        offset = self.q_offset(x).reshape(B,int(N**0.5),int(N**0.5),2)
        offset_range = torch.tensor([1.0 / (int(N**0.5) - 1.0), 1.0 / (int(N**0.5) - 1.0)], device=offset.device).reshape(1, 1, 1, 2)
        offset = offset.tanh().mul(offset_range).mul(self.offset_range_factor)
        reference = self.get_q_grid(int(N**0.5),int(N**0.5),B,offset.dtype,offset.device)
        pos = (offset + reference).clamp(-1., +1.)

        q = torch.nn.functional.grid_sample(
                input=q.reshape(B, int(N**0.5),int(N**0.5),C).permute(0,3,1,2), 
                grid=pos[..., (1, 0)], # y, x -> x, y
                mode='bilinear', align_corners=True).permute(0,2,3,1).reshape(B,N,self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
    ...
    @torch.no_grad()
    def get_q_grid(self, H, W, B, dtype, device):
...
if __name__ == "__main__":
    ...
    model = mit_b2(pretrained = False)
    # model = Block(128, 2, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
    #              drop_path=0., act_layer=GELU, norm_layer=nn.LayerNorm, sr_ratio=1)
    # model = DAttention(128, num_heads=4, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,offset_range_factor=1)
