...
def read_jpg_png(line):
    img,label = line.split(' ')
    jpg = Image.open(img).convert('RGB')
    png = Image.open(label).convert('L')
    return jpg,png
...
if __name__ == "__main__":
    ...
    model_path = "logs/last_epoch_weights_b3_stripe_conv_new_dcn2x_seg_gla.pth"
    #------------------------------#
    #   输入图片的大小
    #------------------------------#
    input_shape     = [512, 512]
    ...
    if model_path != '':
        ...
        if pretrained_dict.get('model') != None:
...
if os.path.exists('./model_data/train_val'):
    ...
    train_lines = read_txt('./model_data/train_val/train_chn6.txt')
    val_lines = read_txt('./model_data/train_val/val_chn6.txt')
    ...
    #         train_jpg_png = list(tqdm(executor.map(read_jpg_png, train_lines), total=len(train_lines)))

    #     with futures.ThreadPoolExecutor(8) as executor:  # 实例化线程池
    #         val_jpg_png = list(tqdm(executor.map(read_jpg_png, val_lines),total=len(val_lines)))

    #     train_jpg_png = np.array(train_jpg_png,dtype=object)
    #     train_jpg,train_png = train_jpg_png[:,0],train_jpg_png[:,1]

    #     del train_jpg_png

    #     val_jpg_png = np.array(val_jpg_png,dtype=object)
    #     val_jpg,val_png = val_jpg_png[:,0],val_jpg_png[:,1]
    ...
    if True:
        ...
        if optimizer_dict != None:
            ...
            if local_rank == 0:
                print('Optimizer state dict is load !')
        ...
        # if not distributed:
        #     train_dataset   = SegmentationDataset_disk(train_jpg,train_png, input_shape, num_classes, True, VOCdevkit_path)
        #     val_dataset     = SegmentationDataset_disk(val_jpg,val_png, input_shape, num_classes, False, VOCdevkit_path)
        # else:
        ...
        if local_rank == 0:
            eval_callback   = EvalCallback(model, input_shape, num_classes, val_lines, VOCdevkit_path, log_dir, Cuda, \
                                            eval_flag=eval_flag, period=eval_period,miou_out_path=".temp_miou_out"+name)
        else:
            eval_callback   = None
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, \
                dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, save_dir, local_rank,phi+name)
