...
@jit(float32[:,:,:](uint8[:,:]),nopython=True)#(nopython=True)
def get_con_1(png):
    img = np.where(png > 0, 1, 0)
    shp = img.shape

    img_pad = np.zeros((shp[0] + 4, shp[0] + 4),dtype=float32)
    img_pad[2:-2, 2:-2] = img
    dir_array0 = np.zeros((shp[0], shp[1], 9),dtype=float32)
...
def get_con_3(png):
    ...
    img_pad = np.zeros((shp[0] + 8, shp[0] + 8),dtype=float32)
    img_pad[4:-4, 4:-4] = img
    dir_array0 = np.zeros((shp[0], shp[1], 9),dtype=float32)
...
class SegmentationDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img,label = self.annotation_lines[index].split(' ')
        #-------------------------------#
        #   从文件中读取图像
        #-------------------------------#
        ...
        return jpg, png,get_con_1(png),get_con_3(png), seg_labels
...
class SegmentationDataset_disk(SegmentationDataset):
    def __init__(self, jpgs,pngs, input_shape, num_classes, train, dataset_path):
        self.jpgs               = jpgs
        self.pngs               = pngs
        self.length             = len(jpgs)
        self.input_shape        = input_shape
        self.num_classes        = num_classes
        self.train              = train
        self.dataset_path       = dataset_path
    ...
    def __getitem__(self, index):
        ...
        # jpg = Image.open(img).convert('RGB')
        # png = Image.open(label).convert('L')
        jpg,png = self.jpgs[index],self.pngs[index]
        #-------------------------------#
        #   数据增强
        #-------------------------------#
        jpg, png    = self.get_random_data(jpg, png, self.input_shape, random = self.train)
