...
if __name__ == "__main__":
    ...
    # model_path = "logs/ep050-loss0.166-val_loss0.133_b2.pth"
    #------------------------------#
    #   输入图片的大小
    #------------------------------#
    input_shape     = [512, 512]
    ...
    if model_path != '':
        ...
        for k, v in pretrained_dict.items():
            if k in model_dict.keys() and np.shape(model_dict[k]) == np.shape(v):
                temp_dict[k] = v
                load_key.append(k)
            elif 'backbone.'+k in model_dict.keys() and np.shape(model_dict['backbone.'+k]) == np.shape(v):
                temp_dict['backbone.'+k] = v
                load_key.append('backbone.'+k)
            elif 'decode_head.'+k in model_dict.keys() and np.shape(model_dict['decode_head.'+k]) == np.shape(v):
                temp_dict['decode_head.'+k] = v
                load_key.append('decode_head.'+k)
            else:
...
if os.path.exists('./model_data/train_val'):
    train_lines = []
    val_lines = []
    for path_temp in os.listdir('./model_data/train_val'):
        if 'train' in path_temp:
            path_temp = os.path.join('./model_data/train_val',path_temp)
            train_lines += read_txt(path_temp)
        else:
            path_temp = os.path.join('./model_data/train_val',path_temp)
            val_lines += read_txt(path_temp)
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, \
                dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, save_dir, local_rank,phi)
